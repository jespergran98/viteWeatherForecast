...
export const getBackgroundImage = (temperature, symbolCode) => {
  ...
  const [weatherType, timeVariant] = symbolCode.split('_');
  const iconNumber = ICON_MAP[weatherType] || '01';

  if (TIME_VARIANT_ICONS.includes(iconNumber)) {
    let variant = 'd';
